...
export const methods = {
  ...
  "confirmNewPaymentAuthorization"(paymentID, payerId) {
    ...
    try {
      const authString =  options.username + ":" + options.password;
      console.log("url: " + options.url + '/v1/payments/payment/' + paymentID + '/execute');
   ...
   * @return {Object} results from PayPal normalized
   */
  "paypalNewExpress/payment/capture"(paymentMethod) {
   ...
   */
  "paypalNewExpress/refund/create"(paymentMethod, amount) {
    check(amount, Number);
   ...
   */
  "paypalNewExpress/refund/list"(paymentMethod) {
    // Call both check and validate because by calling `clean`, the audit pkg
    // thinks that we haven't checked paymentMethod arg
    check(paymentMethod, Object);
    PaymentMethodArgument.validate(PaymentMethodArgument.clean(paymentMethod));
